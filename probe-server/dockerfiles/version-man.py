...
SETTING_FILE_DIR: Path = Path(path.dirname(path.dirname(path.realpath(__file__))))
...
@click.option("--push-commit",
              ...
              required=False,
              default=True,
              help="git commit and push after version upgraded")
...
def build(tag: str,
    ...
    if push_commit:
        system(f"git add {SETTING_FILE_DIR / 'VERSION'}")
